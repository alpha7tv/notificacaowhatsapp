...
<div class="card narrow">
  <form method="post" action="/sistema/configuracoes" class="form">
    ...
    <h2>Envio</h2>
    <label>Intervalo mínimo entre mensagens (segundos)<input type="number" name="send_interval_seconds" min="3" max="300" value="<?= (int) ($s['send_interval_seconds'] ?? 10) ?>"></label>
    <p class="muted small">Recomendado 10s ou mais na Evolution API para reduzir o risco de bloqueio do número. Um atraso aleatório de até 40% é somado.</p>
    <label>Máximo de mensagens por hora<input type="number" name="send_max_per_hour" min="1" max="2000" value="<?= (int) ($s['send_max_per_hour'] ?? 200) ?>"></label>
